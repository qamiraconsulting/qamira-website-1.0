...
export const articles: Article[] = [
  ...
  },
  {
    ...
    summary:
      "Rules-based, judgment-dependent, error-tolerant: the three axes that separate a quick automation win from a recommendation we won't make.",
    datePublished: "2026-07-30",
    body: [
      ...
      "Suitability itself breaks down into a few practical questions. Is the task rules-based, or does it depend on contextual judgment a model can't reliably reconstruct? Is it error-tolerant, where an occasional mistake is a minor inconvenience, or error-intolerant, where a mistake is a compliance or trust problem? Is the volume high enough that automating it actually returns meaningful time, or is it already infrequent enough that a human doing it occasionally is cheaper than building and maintaining an agent for it?",
      "High suitability and high business impact becomes a quick-win recommendation, built now. High impact but lower suitability gets flagged as a roadmap candidate -- worth preparing the ground for, not worth forcing today. Low impact gets monitored, not automated, regardless of how technically easy it would be. And critically, low suitability paired with high judgment-dependency gets an explicit do-not-automate recommendation -- not silence, not a vague \"maybe later,\" an actual documented reason we're advising against it.",
